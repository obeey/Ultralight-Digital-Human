...
def timeout_context(seconds):
    ...
    def timeout_handler(signum, frame):
        raise StreamTimeoutError(f"推流超时 ({seconds} 秒)")
...
class NonBlockingStreamer:
    ...
    def _stream_worker(self, video_path, audio_path):
        """推流工作线程"""
        try:
            logger.info(f"开始推流: {video_path}")
            ...
            # 启动推流
            self.streamer.start_stream(video_queue)
            
            # 监控推流进度
            start_time = time.time()
            while not video_queue.empty():
                if time.time() - start_time > self.timeout:
                    raise StreamTimeoutError(f"推流超时: {self.timeout}秒")
                time.sleep(0.1)
            
            self.stream_result = "success"
            logger.info("推流完成")
    ...
    def wait_for_completion(self, check_interval=1.0):
        ...
        if self.stream_error:
            raise Exception(self.stream_error)
        
        return self.stream_result == "success"
...
def parse_arguments():
    ...
    parser.add_argument("--stream-timeout", type=int, default=300, help="推流超时时间(秒)")
...
def validate_arguments(args):
    ...
    if args.stream_timeout < 10:
        logger.error(f"推流超时时间过短: {args.stream_timeout}秒，建议至少10秒")
        return False
...
def run_single_mode(config_path: str, text: str, output_path: Optional[str] = None,
    ...
    try:
        ...
        if enable_stream:
            try:
                logger.info(f"开始UDP推流: {video_path} (超时: {stream_timeout}秒)")
                streamer = NonBlockingStreamer(config, timeout=stream_timeout)
                
                # 启动非阻塞推流
                if streamer.start_stream(video_path, audio_path):
                    logger.info("推流已启动，等待完成...")
                    
                    # 等待推流完成，带超时控制
                    try:
                        success = streamer.wait_for_completion(check_interval=1.0)
                        if success:
                            logger.info("推流成功完成")
                        else:
                            logger.error("推流失败")
